...
def nodal_project(solution_dns, mesh_les, mesh_dns):
    if len(mesh_dns) != len(solution_dns):
        mesh_dns_eval = np.linspace(mesh_dns[0], mesh_dns[-1], len(solution_dns))
    else:
        mesh_dns_eval = mesh_dns

    return np.interp(mesh_les, mesh_dns_eval, solution_dns)
...
class BurgersDataGenerator(SolverBase):
    ...
    def compute_u_prime(self, interpolated_les_solution):
        """Computes u_prime = u_dns - u_les_projected at the current timestep."""
        # Handle flattened or multi-step history arrays
        if interpolated_les_solution.shape != self.solution.shape:
            if (
                interpolated_les_solution.ndim == 1
                and len(interpolated_les_solution) % len(self.solution) == 0
            ):
                # Extract the most recent spatial snapshot (last N_dns values)
                u_les_current = interpolated_les_solution[-len(self.solution) :]
            elif interpolated_les_solution.ndim == 2:
                # If stored as 2D matrix (time_steps, space), grab current timestep
                u_les_current = interpolated_les_solution[self.time_idx]
            else:
                raise ValueError(
                    f"Cannot broadcast LES solution shape {interpolated_les_solution.shape} "
                    f"to DNS solution shape {self.solution.shape}"
                )
        else:
            u_les_current = interpolated_les_solution

        return self.solution - u_les_current
