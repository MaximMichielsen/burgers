...
class SolverBase:
    ...
    def _format_config_for_display(self) -> dict[str, str]:
        ...
        if self.forcing is None:
            ...
            self.forcing, "__name__"
        ):  # Standard function (e.g. def my_forcing():)
            forcing_str = f"{self.forcing.__name__} (from {getattr(self.forcing, '__module__', '')})"
        else:  # Class instance (e.g. DNSDataForcing instance)
            forcing_str = f"{self.forcing.__class__.__name__} (from {getattr(self.forcing.__class__, '__module__', '')})"
