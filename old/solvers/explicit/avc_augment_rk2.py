...
from old.ml.ml_agents.solver_configs import AVCConfig
from old.solvers.explicit.base_solver_rk2 import BaseRK2

logger = logging.getLogger(__name__)


class AVCSolverRK2(BaseRK2):
    """Burgers FEM solver augmented with AV corrector.
    ...
    def plot_avc_contributions(self) -> None:
        ...
        time_axis = self.time_steps[self._avc_cfg.n_skip_steps : n_plot_points]
        av_raw = self.av_history[:n_plot_points]
        av_array = np.array(
            [np.mean(a) if isinstance(a, np.ndarray) else a for a in av_raw],
            dtype=np.float64,
        )
        av_array = av_array[self._avc_cfg.n_skip_steps :]
        drain_array = np.cumsum(self.energy_drain_history[:n_plot_points])
        drain_array = drain_array[self._avc_cfg.n_skip_steps :]
